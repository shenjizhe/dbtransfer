 ...
 *
 * 名称|类型|描述 -|-|- id |Long | match_id |Long |赛事ID club_type |Integer
 * |产生此事件的球队(1:主场球队,2:客场球队) club_id |Long |球队ID FK -> data_club.id event_type
 * |Integer | 事件类型(0:比赛开始,1:进球,2:助攻,3:红牌,4:黄牌,5:换人,6:比赛结束) event_data |String
 * |赛事事件文本 event_point |Integer |事件发生时间点(如: 比赛中的第 45min ) event_at |Timestamp
 * |事件发生时间 player_id |Long |球员ID player_secondary_id |Long |次要球员ID player_name
 * |String |球员名称 player_secondary_name|String |次要球员名称 created_at |Date |数据记录创建时间
 * updated_at |Date |数据记录更新时间
 */
@Entity
